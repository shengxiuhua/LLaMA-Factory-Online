...
class DataDistributionAnalyzer:
    ...
    这个类提供了全面的数据分布分析功能，帮助理解数据的特征分布，
    并提供智能的数据划分策略。
    """
    
    def __init__(self, data, text_column, label_column=None):
        self.data = data
    ...
    def analyze_label_distribution(self):
        ...
        分析数据集中各个标签的分布情况，识别类别不平衡问题。
        类别不平衡会导致模型偏向多数类，影响少数类的识别效果。
        """
        if not self.label_column:
            return None
        
        label_counts = Counter(self.data[self.label_column])
        total_samples = len(self.data)
        
        distribution_stats = {}
        for label, count in label_counts.items():
            distribution_stats[label] = {
                'count': count,
        ...
        # 计算分布均匀性
        # 均匀性分数越高，表示各类别分布越均匀
        percentages = [stats['percentage'] for stats in distribution_stats.values()]
        uniformity_score = 1 - (np.std(percentages) / np.mean(percentages))
        
        # 识别严重不平衡的类别
        imbalanced_classes = []
        mean_percentage = 100 / len(distribution_stats)
        for label, stats in distribution_stats.items():
            if stats['percentage'] < mean_percentage * 0.3:  # 少于平均值的30%
                imbalanced_classes.append(label)
        
        return {
            'distribution': distribution_stats,
            'uniformity_score': round(uniformity_score, 3),
            'is_balanced': uniformity_score > 0.7,
            'imbalanced_classes': imbalanced_classes,
            'total_classes': len(distribution_stats)
        }
    
    def analyze_text_length_distribution(self):
        """分析文本长度分布
        
        文本长度的分布反映了数据的复杂度和多样性。
        长度分布不均可能影响模型的学习效果。
        """
        lengths = [len(str(text)) for text in self.data[self.text_column]]
        ...
        outliers = {
            'too_short': sum(1 for l in lengths if l < lower_bound),
            'too_long': sum(1 for l in lengths if l > upper_bound),
            'outlier_ratio': round((sum(1 for l in lengths if l < lower_bound or l > upper_bound) / len(lengths)) * 100, 2)
        }
        
        length_stats['outliers'] = outliers
        return length_stats
    
    def analyze_vocabulary_distribution(self):
        """分析词汇分布
        ...
        vocab_stats = {
            ...
            'vocabulary_richness': round(len(set(words)) / len(words), 4),
        ...
        vocab_stats['most_common_words'] = most_common
    ...
    def smart_split(self, test_size=0.2, val_size=0.1, random_state=42):
        ...
        if data_size < 1000:
            if data_size < 100:
                # 极小样本：建议使用交叉验证
                return self._small_sample_split()
            else:
                # 小样本：调整划分比例，减少验证集和测试集的比例
                test_size = min(0.15, test_size)
                val_size = min(0.1, val_size)
        
        if self.label_column:
            # 分层划分保持标签分布
            # 这确保训练集、验证集、测试集中各类别的比例保持一致
            ...
            # 进一步划分验证集和测试集
            temp_data = self.data.iloc[temp_idx]
            val_ratio = val_size / (test_size + val_size)
            
            val_data, test_data = train_test_split(
                temp_data,
                test_size=1-val_ratio,
                stratify=temp_data[self.label_column],
                random_state=random_state
            )
        ...
        split_info = {
            ...
            'train_ratio': round(len(train_data) / data_size, 3),
            'val_ratio': round(len(val_data) / data_size, 3),
            'test_ratio': round(len(test_data) / data_size, 3)
        }
    ...
    def _small_sample_split(self):
        """小样本特殊划分策略
        
        对于极小样本数据集，传统的划分方法可能不适用。
        """
        data_size = len(self.data)
        ...
        if data_size < 50:
            # 极小样本：建议使用交叉验证
            return {
                'recommendation': 'use_cross_validation',
                'reason': f'数据量过小({data_size}条)，建议使用5折交叉验证',
                'suggested_folds': min(5, data_size),
                'alternative': '如果必须划分，建议80-10-10的比例'
            }
        else:
            # 小样本：80-10-10划分
            train_data, temp_data = train_test_split(
            ...
            return {
                ...
                'split_info': {
                    ...
                    'val_size': len(val_data),
                    'test_size': len(test_data),
                    'note': '小样本数据集，建议密切关注过拟合问题'
                }
            }
    
    def validate_split_quality(self, split_result):
        """验证划分质量
        
        检查数据划分是否合理，各个子集的分布是否一致。
        """
        if 'recommendation' in split_result:
            return split_result
        ...
        # 检查各集合的标签分布一致性
        if self.label_column:
            train_dist = Counter(split_result['train'][self.label_column])
            val_dist = Counter(split_result['validation'][self.label_column])
            test_dist = Counter(split_result['test'][self.label_column])
            
            # 计算分布差异
            all_labels = set(train_dist.keys()) | set(val_dist.keys()) | set(test_dist.keys())
            
            for label in all_labels:
                train_ratio = train_dist.get(label, 0) / len(split_result['train'])
                val_ratio = val_dist.get(label, 0) / len(split_result['validation'])
                test_ratio = test_dist.get(label, 0) / len(split_result['test'])
                
                max_diff = max(abs(train_ratio - val_ratio), 
                              abs(train_ratio - test_ratio), 
                              abs(val_ratio - test_ratio))
                
                if max_diff > 0.15:  # 15%的差异阈值
                    issues.append(f"标签'{label}'在各集合中分布差异过大({max_diff:.2%})")
                elif max_diff > 0.1:  # 10%的差异警告阈值
                    warnings.append(f"标签'{label}'在各集合中分布略有差异({max_diff:.2%})")
        
        # 检查集合大小合理性
        split_info = split_result['split_info']
        
        if split_info['val_size'] < 50 and len(self.data) > 500:
            issues.append("验证集过小，可能影响模型选择的可靠性")
        elif split_info['val_size'] < 30:
            warnings.append("验证集较小，建议谨慎解释验证结果")
        
        if split_info['test_size'] < 100 and len(self.data) > 1000:
            issues.append("测试集过小，可能影响性能评估的可靠性")
        elif split_info['test_size'] < 50:
            warnings.append("测试集较小，性能评估可能不够稳定")
        
        # 计算质量分数
        quality_score = 100
        quality_score -= len(issues) * 15  # 严重问题扣15分
        quality_score -= len(warnings) * 5  # 警告扣5分
        ...
        return {
            ...
            'recommendation': self._generate_split_recommendation(issues, warnings)
        }
    
    def _generate_split_recommendation(self, issues, warnings):
        """生成划分建议"""
        if not issues and not warnings:
            return "数据划分质量良好，可以直接使用"
        
        recommendations = []
        
        if any("分布差异" in issue for issue in issues):
            recommendations.append("建议使用分层采样确保各集合标签分布一致")
        
        if any("过小" in issue for issue in issues):
            recommendations.append("建议增加数据量或调整划分比例")
        
        if any("验证集" in issue for issue in issues):
            recommendations.append("考虑使用交叉验证替代固定验证集")
        
        return "; ".join(recommendations) if recommendations else "建议重新检查数据划分策略"
...
sample_data = pd.DataFrame({
    ...
    'user_type': ['new_user'] * 200 + ['old_user'] * 400,
    'purchase_time': ['morning'] * 150 + ['afternoon'] * 200 + ['evening'] * 250
})

analyzer = DataDistributionAnalyzer(sample_data, 'text', 'label')

# 分析标签分布
print("=== 标签分布分析 ===")
label_dist = analyzer.analyze_label_distribution()
print(f"标签分布: {label_dist['distribution']}")
print(f"分布均匀性: {label_dist['uniformity_score']}")
print(f"是否平衡: {label_dist['is_balanced']}")
if label_dist['imbalanced_classes']:
    print(f"不平衡类别: {label_dist['imbalanced_classes']}")

# 分析文本长度分布
print("\n=== 文本长度分布分析 ===")
length_dist = analyzer.analyze_text_length_distribution()
print(f"平均长度: {length_dist['mean_length']}")
print(f"长度范围: {length_dist['min_length']} - {length_dist['max_length']}")
print(f"长度分位数: {length_dist['percentiles']}")
print(f"异常值比例: {length_dist['outliers']['outlier_ratio']}%")

# 分析词汇分布
print("\n=== 词汇分布分析 ===")
vocab_dist = analyzer.analyze_vocabulary_distribution()
print(f"总词数: {vocab_dist['total_words']}")
print(f"唯一词数: {vocab_dist['unique_words']}")
print(f"词汇丰富度: {vocab_dist['vocabulary_richness']}")

# 智能划分数据集
print("\n=== 数据集划分 ===")
split_result = analyzer.smart_split()
print(f"划分结果: {split_result['split_info']}")

# 验证划分质量
validation_result = analyzer.validate_split_quality(split_result)
print(f"划分质量评分: {validation_result['quality_score']}/100")
if validation_result['issues']:
    print(f"发现问题: {validation_result['issues']}")
if validation_result['warnings']:
    print(f"注意事项: {validation_result['warnings']}")
